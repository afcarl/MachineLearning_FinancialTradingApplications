# ML4T - MC2P2 - PART II 'MY STRATEGY'
# Mark Trinquero 

# Strategy overview:  used SPY and MSFT as additional indicators for IBM trading.  
# SPY indicator only triggers IBM short sells (500 shares) based on large market movements 
# MSFT indicator servers as proxy for tech sector and triggers IBM shorts/longs (50 shares)
...
def get_longs(lower_band, rolling_mean_IBM, df):
    ...
    long_entries = np.where(entry_detector == 1)
    entries = get_long_data(long_entries)
    
    # Long Exits = crosses rolling_mean from below
    exit_detector = get_indicator_data(rolling_mean_IBM, df)
    long_exits = np.where(exit_detector == 1)
    exits = get_long_data(long_exits)
...
#### MY STRATEGY - SPY INDICATOR 

def get_indicator_data1(metric, df1):
    detector = np.pad(np.diff(np.array(df1['SPY'] > metric).astype(int)), (1, 0), 'constant', constant_values=(0, ))
    return detector


def get_shorts1(upper_band_SPY, rolling_mean_SPY, df1):
    global counter
    global trades_df

    # Short Entries= crosses Upper Band from above
    entry_detector = get_indicator_data1(upper_band_SPY, df1)
    entries = get_short_data(np.where(entry_detector == -1))    
    # Short Exits= crosses rolling_mean from above
    exit_detector = get_indicator_data1(rolling_mean_SPY, df1)
    exits = get_short_data(np.where(exit_detector == -1))
    ...
    while i < len(entries):
        while entries[i] < current_exit:
            i += 1
        plt.axvline(df1.iloc[entries[i]].name, color='r') # RED bar for short entry
        trades_df.loc[counter] = [df1.iloc[entries[i]].name, 'IBM', 'SELL', 500] # trigger ibm short sell per strategy
        counter += 1
        current_entry = entries[i]
        while j < len(exits):
            if exits[j] > current_entry:
                plt.axvline(df1.iloc[exits[j]].name, color='k') # BLACK bar for short exit
                trades_df.loc[counter] = [df1.iloc[exits[j]].name, 'IBM', 'BUY', 500] # trigger ibm buy per strategy
                counter += 1
                current_exit = exits[j]
                break
            j += 1
...
def get_longs1(lower_band_SPY, rolling_mean_SPY, df1):
    global counter
    global trades_df

    # Long Entries= crosses Lower Band from below
    entry_detector = get_indicator_data1(lower_band_SPY, df1)
    ...
    while i < len(entries):
        while entries[i] < current_exit:
            i += 1
        plt.axvline(df1.iloc[entries[i]].name, color='g') # GREEN bar for long entry
        trades_df.loc[counter] = [df1.iloc[entries[i]].name, 'IBM', 'BUY', 500] #trigger ibm buy per strategy
        counter += 1
        current_entry = entries[i]
        while j < len(exits):
            if exits[j] > current_entry:
                plt.axvline(df1.iloc[exits[j]].name, color='k') # BLACK bar for long exit
                trades_df.loc[counter] = [df1.iloc[exits[j]].name, 'IBM', 'SELL', 500] #trigger ibm sell per strategy
                counter += 1
                current_exit = exits[j]
                break
            j += 1
...
#### MY STRATEGY - MSFT INDICATOR (Tech Sector Proxy)


def get_indicator_data2(metric, df2):
    detector = np.pad(np.diff(np.array(df2['MSFT'] > metric).astype(int)), (1, 0), 'constant', constant_values=(0, ))
    return detector


def get_shorts2(upper_band_MSFT, rolling_mean_MSFT, df2):
    global counter
    global trades_df

    # Short Entries= crosses Upper Band from above
    entry_detector = get_indicator_data2(upper_band_MSFT, df2)
    entries = get_short_data(np.where(entry_detector == -1))    
    # Short Exits= crosses rolling_mean from above
    exit_detector = get_indicator_data2(rolling_mean_MSFT, df2)
    exits = get_short_data(np.where(exit_detector == -1))
    ...
    while i < len(entries):
        while entries[i] < current_exit:
            i += 1
        plt.axvline(df2.iloc[entries[i]].name, color='r') # RED bar for short entry
        trades_df.loc[counter] = [df2.iloc[entries[i]].name, 'IBM', 'SELL', 500] # trigger ibm short sell per strategy
        counter += 1
        current_entry = entries[i]
        while j < len(exits):
            if exits[j] > current_entry:
                plt.axvline(df2.iloc[exits[j]].name, color='k') # BLACK bar for short exit
                trades_df.loc[counter] = [df2.iloc[exits[j]].name, 'IBM', 'BUY', 500] # trigger ibm buy per strategy
                counter += 1
                current_exit = exits[j]
                break
            j += 1
...
def get_longs2(lower_band_MSFT, rolling_mean_MSFT, df2):
    global counter
    global trades_df

    # Long Entries= crosses Lower Band from below
    entry_detector = get_indicator_data2(lower_band_MSFT, df2)
    entries = get_long_data(np.where(entry_detector == 1))
    ...
    while i < len(entries):
        while entries[i] < current_exit:
            i += 1
        plt.axvline(df2.iloc[entries[i]].name, color='g') # GREEN bar for long entry
        trades_df.loc[counter] = [df2.iloc[entries[i]].name, 'IBM', 'BUY', 500] #trigger ibm buy per strategy
        counter += 1
        current_entry = entries[i]
        while j < len(exits):
            if exits[j] > current_entry:
                plt.axvline(df2.iloc[exits[j]].name, color='k') # BLACK bar for long exit
                trades_df.loc[counter] = [df2.iloc[exits[j]].name, 'IBM', 'SELL', 500] #trigger ibm sell per strategy
                counter += 1
                current_exit = exits[j]
                break
            j += 1
...
############################################################
# MY STRATEGY - add heuristics to trade IBM based off of SPY/MSFT movements
    ...
    df1 = get_data(indicator1, dates)
    df2 = get_data(indicator2, dates)
    # IBM rolling_mean (rolling mean)
    rolling_mean_IBM = get_rolling_mean(df['IBM'], window=20)
    rolling_mean_SPY = get_rolling_mean(df1['SPY'], window=18)
    rolling_mean_MSFT = get_rolling_mean(df2['MSFT'], window=10)

    # IBM rolling standard deviation
    rolling_std_IBM = get_rolling_std(df['IBM'], window=20)
    rolling_std_SPY = get_rolling_std(df1['SPY'], window=18)
    rolling_std_MSFT = get_rolling_std(df2['MSFT'], window=10)
    # Upper and Lower Bollinger Bands
    upper_band, lower_band = get_bollinger_bands(rolling_mean_IBM, rolling_std_IBM)
    upper_band_SPY, lower_band_SPY = get_bollinger_bands(rolling_mean_SPY, rolling_std_SPY)
    upper_band_MSFT, lower_band_MSFT = get_bollinger_bands(rolling_mean_MSFT, rolling_std_MSFT)
    ...
    ax1 = df1['SPY'].plot(title="Bollinger Bands", label='SPY', color='y')
    rolling_mean_SPY.plot(label='SMA SPY', ax=ax, color='m')
    upper_band.plot(label='Upper Band SPY', ax=ax, color='k')
    lower_band.plot(label='Lower Band SPY', ax=ax, color='k')

    ax2 = df2['MSFT'].plot(title="Bollinger Bands", label='MSFT', color='y')
    rolling_mean_MSFT.plot(label='SMA MSFT', ax=ax, color='m')
    upper_band.plot(label='Upper Band MSFT', ax=ax, color='k')
    lower_band.plot(label='Lower Band MSFT', ax=ax, color='k')
    ...
    # ADD MY STRATEGY ENTRIES/ EXITS

    # Get short entries/exits
    get_shorts1(upper_band_SPY, rolling_mean_SPY, df1)
    # Get long entries/exits
    #get_longs1(lower_band_SPY, rolling_mean_SPY, df1)

    # Get short entries/exits
    get_shorts2(upper_band_MSFT, rolling_mean_MSFT, df2)
    # Get long entries/exits
    get_longs2(lower_band_MSFT, rolling_mean_MSFT, df2)    
